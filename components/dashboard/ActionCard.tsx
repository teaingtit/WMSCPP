...
export function ActionCard({ href, icon, tag, title, description, variant = 'secondary' }: ActionCardProps) {
  const baseClasses = "group rounded-3xl p-5 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] transition-all transform hover:-translate-y-1 hover:scale-[1.02] hover:brightness-110 flex items-center gap-5 relative overflow-hidden backdrop-blur-md border border-white/10";
  ...
  if (tag === 'INBOUND') {
      colorClasses = "bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-800 shadow-emerald-900/40";
      iconBgClass = "bg-gradient-to-b from-white/20 to-white/5 text-white shadow-[inset_0_1px_1px_rgba(255,255,255,0.4)] border border-white/20";
      ...
      colorClasses = "bg-gradient-to-br from-slate-700 via-slate-800 to-slate-900 shadow-slate-900/40";
      iconBgClass = "bg-gradient-to-b from-white/10 to-white/5 text-slate-200 shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] border border-white/10";
  }
      ...
      <div className={`${baseClasses} ${colorClasses}`}>
        ...
        <div className="flex-1 min-w-0 z-10">
            <div className="flex items-center gap-2 mb-1">
                <span className="text-[10px] font-black text-white/80 uppercase tracking-widest drop-shadow-md">{tag}</span>
            </div>
