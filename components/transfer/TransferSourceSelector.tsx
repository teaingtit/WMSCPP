...
export default function TransferSourceSelector({ warehouseId, activeTab, selectedStock, onSelect }: Props) {
  ...
  const themeColors = {
      INTERNAL: { text: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200', ring: 'focus:ring-orange-500/20', borderFocus: 'focus:border-orange-500' },
      CROSS: { text: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-200', ring: 'focus:ring-indigo-500/20', borderFocus: 'focus:border-indigo-500' }
  }[activeTab];
  ...
  return (
    <div className={`p-6 rounded-3xl border transition-all bg-white shadow-sm duration-300 ${selectedStock ? themeColors.bg + ' ' + themeColors.border : 'border-slate-200'}`}>
      ...
      {!selectedStock ? (
                ...
                >
                  <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="font-bold text-slate-700 group-hover:text-slate-900 mb-1">{stock.products.name}</div>
                      ...
                      <div className="text-right pl-4">
                         <div className={`font-black text-lg ${themeColors.text}`}>
                             {stock.quantity.toLocaleString()} <span className="text-[10px] font-bold text-slate-400 uppercase">{stock.products.uom}</span>
                         </div>
        ...
        <div className="relative animate-in fade-in zoom-in-95">
           <div className={`p-5 rounded-2xl bg-white border border-white/50 shadow-sm flex flex-col md:flex-row gap-4 items-start md:items-center justify-between`}>
              
              {/* Product Info */}
              <div className="flex-1">
                  <div className={`text-[10px] font-bold uppercase tracking-wider mb-1 opacity-70 ${themeColors.text}`}>Source Stock</div>
                  <div className="font-black text-slate-800 text-xl leading-snug">{selectedStock.products.name}</div>
                  <div className="text-sm text-slate-500 font-mono mt-0.5">{selectedStock.products.sku}</div>
              </div>

              {/* Stats Grid */}
              <div className="flex items-stretch gap-2 w-full md:w-auto">
                 <div className={`px-4 py-2 rounded-xl border flex flex-col items-center justify-center min-w-[80px] ${themeColors.bg} ${themeColors.border}`}>
                 ...
                 <div className={`px-4 py-2 rounded-xl border flex flex-col items-center justify-center min-w-[80px] bg-slate-800 border-slate-700 text-white shadow-lg`}>
                    <span className="text-[10px] uppercase font-bold text-slate-400">Available</span>
                    <div className="font-black text-lg">
                        {selectedStock.quantity} <span className="text-[10px] font-normal opacity-70">{selectedStock.products.uom}</span>
                    </div>
                 </div>
              </div>
