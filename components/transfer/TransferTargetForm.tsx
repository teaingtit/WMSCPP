...
import { MapPin, Save, Loader2, CheckCircle2, ArrowRight, Plus, Trash2, ListChecks, Package } from 'lucide-react';
...
export default function TransferTargetForm({ sourceStock, currentWarehouseId, activeTab, warehouses, onClearSelection }: Props) {
  ...
  return (
    <div className="space-y-6">
       <div className={`bg-white p-6 rounded-3xl shadow-sm border border-slate-200 transition-all ${isFormDisabled ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
       ...
       {queue.length > 0 && (
        <div className="bg-slate-900 text-white p-6 rounded-3xl shadow-xl animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-2 mb-6">
                {queue.map((item, idx) => (
                    <div key={item.id} className="bg-slate-800 p-3 rounded-xl border border-slate-700 flex justify-between items-start group">
                        <div className="flex-1 min-w-0 pr-2">
                            <div className="font-bold text-sm truncate text-slate-200">{idx + 1}. {item.sourceStock.product.name}</div>
                            <div className="flex items-center gap-2 text-xs text-slate-400 font-mono mt-1">
                                <span className="bg-slate-700 px-1 rounded">{item.sourceStock.location.code}</span>
                                <ArrowRight size={12}/>
                                <span className="bg-indigo-900/50 text-indigo-300 px-1 rounded border border-indigo-500/30">{item.targetLocation.code}</span>
                            </div>
                        </div>
          ...
          details={
            <div className="flex flex-col gap-2 text-sm">
                ...
                <div className="flex justify-between">
                    ...
                    <span className="font-medium text-slate-900">{activeTab === 'INTERNAL' ? 'Internal Transfer' : 'Cross Transfer'}</span>
                </div>
            </div>
          }
       />
