      ...
      <div className={cn(
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            {user && (
                <div className="bg-white/5 rounded-2xl p-4 border border-white/10 shadow-sm">
                    <div className="flex items-center gap-3 mb-3">
                         <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 shadow-sm ${isAdmin ? 'border-primary/20 bg-primary/10 text-primary' : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400'}`}>
                            <UserCircle size={28}/>
                        </div>
                    ...
                    {!isAdmin && (
                        <div className="flex items-center gap-2 text-xs text-slate-400 bg-white/5 p-2 rounded-xl border border-white/5">
                            ...
                            <span>Access: <span className="font-bold text-slate-300">{user.allowed_warehouses?.length || 0}</span> Warehouses</span>
                        </div>
                    )}
                </div>
            )}
            ...
            <nav className="space-y-1">
                <div className="px-2 text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Applications</div>
                    ...
                    return (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <item.icon size={20} className={isActive ? "text-primary-foreground/90" : "text-slate-500 group-hover:text-primary"} />
                                <span>{item.title}</span>
                            </div>
                            {!isActive && <ChevronRight size={16} className="text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity" />}
                        </Link>
                    );
                })}
            </nav>
        ...
        <div className="p-4 border-t border-white/10 bg-slate-900">
            <form action={logout}>
                <button type="submit" className="flex items-center justify-center gap-2 w-full p-3.5 text-rose-400 bg-white/5 border border-white/10 hover:bg-rose-500/10 hover:border-rose-500/30 rounded-xl font-bold transition-all shadow-sm active:scale-[0.98]">
                    <LogOut size={18} /> 
                    <span>Sign Out</span>
                </button>
            </form>
