...
import { LogOut, Package, UserCircle, Shield, Warehouse, LayoutGrid, Settings, ClipboardList, ChevronDown } from 'lucide-react';
...
function NavDropdown({ title, icon: Icon, children, isActive }: { title: string, icon: any, children: React.ReactNode, isActive?: boolean }) {
        ...
        <div className="relative" ref={dropdownRef}>
            <button 
                ...
                className={cn(
                    ...
                    isActive || isOpen ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5 hover:text-white"
                )}
            >
                <Icon size={18} />
                <span>{title}</span>
    ...
    <header className="bg-slate-900/60 backdrop-blur-xl border-b border-white/10 hidden md:flex items-center justify-between px-6 py-3 sticky top-0 z-40 shadow-sm">
      ...
      <div className="flex items-center gap-8">
        ...
        <Link href="/dashboard" className="flex items-center gap-2.5 group">
            ...
            <div>
                <h1 className="font-bold text-white text-lg tracking-tight leading-none">{APP_CONFIG.name}</h1>
            </div>
        </Link>
        ...
        <nav className="flex items-center gap-1">
            ...
            {warehouseId && (
                <NavDropdown 
                    title="Operations" 
                    icon={ClipboardList}
                    isActive={['/audit', '/history', '/inventory', '/inbound', '/outbound'].some(path => pathname.includes(path))}
                >
                    {renderLink(MENU_ITEMS.find(m => m.matchPath === '/inventory') || { title: 'Inventory', href: `/dashboard/${warehouseId}/inventory`, icon: Package, matchPath: '/inventory' } as any)}
                    {MENU_ITEMS.filter(m => ['/audit', '/history'].includes(m.matchPath)).map(m => renderLink(m))}
                </NavDropdown>
            )}
      ...
      <div className="flex items-center gap-4">
        {user && (
            <div className="flex items-center gap-3 pl-4 border-l border-white/10">
                ...
                <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 shadow-sm ${isAdmin ? 'border-primary/20 bg-primary/10 text-primary' : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400'}`}>
