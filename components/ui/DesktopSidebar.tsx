...
import { LogOut, Package, UserCircle, Shield, Warehouse, LayoutGrid, Settings, ClipboardList } from 'lucide-react';
  ...
  return (
    <aside className="w-64 bg-gradient-to-b from-slate-900 to-slate-950 text-white hidden md:flex flex-col shrink-0 h-full shadow-2xl z-50 border-r border-slate-800/50">
      ...
      <div className="p-6 border-b border-slate-800/50 flex items-center gap-3 bg-slate-900/50">
        ...
        <div>
            <h1 className="font-bold text-lg tracking-tight leading-none text-slate-100">{APP_CONFIG.name}</h1>
      ...
      <nav className="flex-1 py-6 space-y-2 overflow-y-auto custom-scrollbar">
        ...
        {warehouseId && (
            <SidebarAccordion title="Operations" icon={ClipboardList} defaultOpen={true}>
                {MENU_ITEMS.filter(m => ['/audit', '/history', '/inventory'].includes(m.matchPath) && !m.hidden).map(renderLink)}
        ...
        {user && (
            <div className="bg-slate-800/40 rounded-xl p-3 mb-3 border border-slate-700/30 backdrop-blur-sm">
                <div className="flex items-center gap-3 mb-2">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 shadow-sm ${isAdmin ? 'border-purple-500/50 bg-purple-500/10 text-purple-400' : 'border-emerald-500/50 bg-emerald-500/10 text-emerald-400'}`}>
