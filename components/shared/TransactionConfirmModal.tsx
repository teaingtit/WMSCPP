...
export default function TransactionConfirmModal({
  ...
  const themes = {
    INBOUND: { 
        bg: 'bg-emerald-600', text: 'text-emerald-600', border: 'border-emerald-100', bgLight: 'bg-emerald-50',
        Icon: PackagePlus
    },
    TRANSFER: { 
        bg: 'bg-indigo-600', text: 'text-indigo-600', border: 'border-indigo-100', bgLight: 'bg-indigo-50',
        Icon: ArrowRightLeft
    },
    OUTBOUND: { 
        bg: 'bg-rose-600', text: 'text-rose-600', border: 'border-rose-100', bgLight: 'bg-rose-50',
        Icon: Truck
    },
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100">
        ...
        <div className={`${theme.bg} p-6 text-white flex justify-between items-start`}>
          <div className="flex gap-3">
            ...
            <div>
                <h3 id="transaction-confirm-modal-title" className="text-xl font-bold">ยืนยันการทำรายการ</h3>
                <p className="text-white/80 text-sm">{title}</p>
            </div>
          </div>
          <button onClick={onClose} disabled={isLoading} aria-label="Close modal" className="text-white/70 hover:text-white p-1 rounded-full hover:bg-white/10 transition-colors">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="p-6">
            ...
            <div className="space-y-3">
                ...
                {onSaveAndContinue && (
                  ...
                  >
                      {isLoading ? <Loader2 className="animate-spin text-slate-400"/> : <Save className="text-slate-500"/>}
