    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "⚠️ ADMIN: Execute modification queries (INSERT, UPDATE, DELETE, CREATE, DROP). Use with extreme caution.",
                inputSchema: {
                    type: "object",
                    properties: {
    ...
    try {
        if (name === "query") {
            ...
            if (!upperSql.startsWith("SELECT") && !upperSql.startsWith("WITH") && !upperSql.startsWith("EXPLAIN")) {
                throw new McpError(ErrorCode.InvalidParams, "The 'query' tool allows SELECT statements only. Use 'execute_write_operation' for modifications.");
            }
