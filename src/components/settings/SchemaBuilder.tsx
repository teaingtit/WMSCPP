  ...
  return (
    <div className="space-y-4 bg-slate-50 p-5 rounded-xl border border-slate-200">
      ...
      {fields.map((field, idx) => (
        <div
          key={idx}
          className="flex flex-col gap-3 p-4 bg-white border border-slate-200 rounded-lg shadow-sm animate-in slide-in-from-left-2 duration-300"
        >
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-slate-600 bg-slate-50 p-2.5 rounded-lg border border-slate-100">
            ...
            <div className="flex items-center gap-3 border-r border-slate-200 pr-4 mr-2">
              ...
              <label
                className={`flex items-center gap-1.5 cursor-pointer px-2 py-1 rounded transition-colors ${
                  field.scope === 'LOT'
                    ? 'bg-emerald-100 text-emerald-700 font-bold ring-1 ring-emerald-200'
                    : 'hover:bg-slate-200 text-slate-500'
                }`}
              >
                <input
