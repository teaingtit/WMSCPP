  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            สรุปภาพรวมความแม่นยำและผลต่างของการตรวจนับ
          </p>
        </div>
      </div>
      ...
      {varianceItems.length === 0 ? (
        <div className="text-center p-8 border-2 border-dashed rounded-lg text-muted-foreground border-border flex flex-col items-center gap-2">
          <CheckCircle className="w-8 h-8 text-green-500" />
          <p>ไม่พบผลต่างของสต็อก หรือยังไม่มีการนับ</p>
        </div>
      ) : (
        <div className="space-y-2">
          <h4 className="font-semibold text-foreground">รายการผลต่าง (Variance Items)</h4>
          ...
          <div className="block md:hidden space-y-3">
            {varianceItems.map((item) => (
              <div
                key={item.id}
                className="bg-card p-4 rounded-xl shadow-sm border border-border space-y-3"
              >
                ...
                <div className="grid grid-cols-3 gap-2 pt-2 border-t border-border text-sm">
                  <div>
                    <div className="text-xs text-muted-foreground uppercase font-bold">System</div>
                    <div className="font-mono text-muted-foreground">{item.system_qty}</div>
                  </div>
                  <div>
                    <div className="text-xs text-muted-foreground uppercase font-bold">Counted</div>
                    <div className="font-mono font-bold text-foreground">{item.counted_qty}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-muted-foreground uppercase font-bold">Diff</div>
                    <div
          ...
          {/* Desktop Table View */}
          <div className="hidden md:block overflow-x-auto custom-scrollbar border border-border rounded-xl shadow-sm bg-card">
              ...
              <TableBody>
                {varianceItems.map((item) => (
                  <TableRow key={item.id} className="hover:bg-accent">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1 text-muted-foreground bg-muted px-2 py-1 rounded w-fit text-xs font-medium">
                        <MapPin size={12} /> {item.location?.code}
                      </div>
                    </TableCell>
