        ...
        return (
            <div className="max-w-7xl mx-auto pb-20 p-4 sm:p-6 animate-in slide-in-from-right-4 duration-300">
                 <div className="flex items-center gap-4 mb-6">
                    <button onClick={() => setIsCountingMode(false)} aria-label="ย้อนกลับ" className="p-2 bg-white border border-slate-200 hover:bg-slate-50 rounded-full transition-colors shadow-sm">
        ...
        <div className="max-w-7xl mx-auto pb-20 p-4 sm:p-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    <button onClick={() => router.back()} aria-label="ย้อนกลับ" className="p-2 hover:bg-slate-100 rounded-full transition-colors">
                        <ArrowLeft className="text-slate-600" />
                    </button>
                    <div>
                        <h1 className="text-2xl font-black text-slate-800">{session.name}</h1>
                        <div className="flex items-center gap-2 text-sm text-slate-500">
                            <span className={`px-2 py-0.5 rounded text-xs font-bold ${session.status === 'OPEN' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                                {session.status}
                            </span>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                <TabsList className="bg-slate-100 p-1 rounded-xl h-auto">
                    <TabsTrigger value="pending" className="flex-1 sm:flex-none rounded-lg px-4 py-2 data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all">
                        <Clock className="w-4 h-4 mr-2 text-amber-500" />
                        รอการนับ ({pendingItems.length})
                    </TabsTrigger>
                    <TabsTrigger value="completed" className="flex-1 sm:flex-none rounded-lg px-4 py-2 data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all">
                            ...
                            <div key={item.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-3">
                                <div className="flex justify-between items-start">
                                    ...
                                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold inline-flex items-center gap-1 ${
                                        item.status === 'COUNTED' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                                    }`}>
                                        ...
                                        <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="p-4 text-center">
                                                <span className={`px-2 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1 ${
                                                    ...
                                                    {item.status === 'COUNTED' ? <CheckCircle2 size={12} /> : <Clock size={12} />}
                                                    {item.status === 'COUNTED' ? 'นับแล้ว' : 'รอการนับ'}
                                                </span>
                                            </td>
                ...
                <div className="mt-8 p-6 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col items-center text-center gap-4">
                    <div className="space-y-1">
                        ...
                        <p className="text-sm text-slate-500 max-w-md">
                            เมื่อยืนยันการปิดรอบ ระบบจะทำการปรับยอดสต็อก (Adjustment) ตามผลต่างที่นับได้ทันที และจะไม่สามารถแก้ไขข้อมูลได้อีก
                        </p>
                    </div>
                    <Button 
                    ...
                    >
                        {isFinalizing ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Lock className="w-4 h-4 mr-2" />}
