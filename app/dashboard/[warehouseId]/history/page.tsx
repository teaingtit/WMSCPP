      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
           ...
           <p className="text-slate-500 text-sm mt-1">
              แสดงความเคลื่อนไหวล่าสุดของคลัง: <span className="font-bold text-indigo-600">{params.warehouseId}</span>
           </p>
        </div>
        <div className="text-right">
             <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Transactions</div>
                  ...
                  <tr key={log.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4">
                       <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${
                           log.type === 'INBOUND' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                           log.type === 'OUTBOUND' ? 'bg-rose-50 text-rose-600 border-rose-100' :
                    ...
                    <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-600 truncate max-w-[100px]" title={log.user}>
                                {log.user.split('@')[0]}
                            </span>
                        </div>
                    </td>
