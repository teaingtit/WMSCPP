...
const quickActions = (warehouseId: string) => [
  {
    ...
    tag: 'INBOUND',
    title: 'รับสินค้าเข้า',
    description: 'บันทึกสินค้าใหม่ / สแกนรับของ',
  ...
  },
  {
    ...
    description: 'ตัดสต็อก / เบิกใช้งาน',
    color: 'bg-rose-500',
  ...
  },
  {
    ...
    tag: 'TRANSFER',
    title: 'ย้ายตำแหน่ง',
    description: 'ย้ายตำแหน่งภายในคลัง',
    color: 'bg-blue-500',
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-fade-in">
        <div>
          ...
          <p className="text-slate-500 text-xs md:text-sm font-medium flex items-center gap-1">
            Warehouse:{' '}
            <span className="bg-white text-slate-700 px-2 py-0.5 rounded font-mono font-bold border border-slate-200 shadow-sm">
              {params['warehouseId']}
            </span>
          </p>
        </div>
        <div className="w-full md:w-auto">
          <Link
            href={`/dashboard/${params['warehouseId']}/inventory`}
            className="flex items-center justify-center gap-2 w-full md:w-auto bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-xl text-sm font-bold shadow-sm hover:bg-slate-50 hover:text-blue-600 hover:border-blue-200 transition-all"
          >
        ...
        {quickActions(params['warehouseId']).map((action) => (
          <Link key={action.tag} href={action.href} className="block group">
            <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex items-center gap-4 h-full">
              <div
                className={`w-12 h-12 rounded-lg flex items-center justify-center text-white shadow-sm shrink-0 ${action.color}`}
              >
                {action.icon}
              </div>
              <div>
                <span
                  className={`text-[10px] font-bold uppercase tracking-widest block mb-0.5 ${action.textColor}`}
                >
                  {action.tag}
                </span>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all flex items-center gap-4 h-full">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs font-medium text-slate-500 mt-0.5">
                ดูรายการสต็อก ({stats.totalItems})
              </p>
            </div>
          </div>
        </Link>
