...
export const createAuditSession = withAuth(
  async (formData: FormData, { user, supabase }) => {
    const rawData = {
    ...
    try {
      ...
      if (existingSession) {
        return {
          success: false,
          message: `ชื่อรอบการนับ "${name}" มีอยู่แล้วในคลังสินค้านี้ กรุณาใช้ชื่ออื่น`,
        };
      }
      ...
      if (selectedItems.length > 0) {
        ...
        auditItemsPayload = selectedItems
          .map((item: any) => {
      ...
      if (auditItemsPayload.length > 0) {
        const { error: itemsError } = await supabase.from('audit_items').insert(auditItemsPayload);
...
export async function getAuditSessionById(sessionId: string) {
  const supabase = await createClient();
  const { data } = await supabase.from('audit_sessions').select('*').eq('id', sessionId).single(); // Use single() as we expect exactly one result
...
export async function updateAuditSession(
  ...
  try {
    const { error } = await supabase.from('audit_sessions').update(data).eq('id', sessionId);
